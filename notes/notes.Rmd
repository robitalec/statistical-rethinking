...
Cafe visits in morning and afternoone, intercepts: average morning wait, slopes: avg difference between afternoon and morning. 

AAre the slopes and intercepts related? Yes. There is pooling across parameters.
...
Simpler to do for univariate models, since we need to factor all parameters out of the prior and into the linear model. How do we factor out a correlation matrix?
...
Gaussian process regression considers infite numbers of categories. (Need pooling)
...
Brownian motion is the oldest and most conservative assumption. The decline in covariance is linear since the date
