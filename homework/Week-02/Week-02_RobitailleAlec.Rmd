...
ggplot(DT) + 
	...
	geom_point(aes(exp(logweight), height), data = d, color = 'blue')
```
...
# Plot
ggplot(DT) + 
	geom_ribbon(aes(x = exp(logweight), ymin = low, ymax = high), color = 'grey') + 
	geom_point(aes(exp(logweight), meanheight)) + 
	geom_point(aes(exp(logweight), height), data = d, color = 'blue')
```
```
