...
simmed <- sim(m, list(weight_s = sim_x, weight_s2 = sim_x ^ 2))
DT <- melt(as.data.table(simmed), value.name = 'height', variable.name = 'x')
DT[data.table(sim_x, x = paste0('V', seq.int(n))),
	 c('weight_s', 'weight_s2') := .(sim_x, sim_x ^ 2),
	 on = 'x']

DT[, meanheight := mean(height), by = weight_s]
DT[, low := PI(height)[1], by = weight_s]
DT[, high := PI(height)[2], by = weight_s]

# Plot
ggplot(DT) + 
	geom_ribbon(aes(x = weight_s, ymin = low, ymax = high), fill = 'grey') + 
	geom_point(aes(weight_s, height), data = d, color = 'lightblue', alpha = 0.8) + 
	geom_line(aes(weight_s, meanheight))
```
