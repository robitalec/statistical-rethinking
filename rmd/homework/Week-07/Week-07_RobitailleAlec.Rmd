...
```{cmdstan, output.var = 'model2_gender'}
data {
  int N;
  int K;
  int N_edu;
...
model_data <- c(
	as.list(DT[, .(response, action, intention, contact, education, age = as.numeric(scale(age))), female = ifelse(gender == 'male', 0, 1)]), 
	N = DT[, .N],
	K = DT[, uniqueN(response) - 1],
	N_edu = DT[, uniqueN(education)],
	alpha = list(rep(2, 7))
