...
```{r}
NWOGrants[, scale_awards := as.numeric(scale(awards))]
NWOGrants[, index_gender := .GRP, gender]
NWOGrants[, index_discipline := .GRP, discipline]

q1_data <- c(
	as.list(NWOGrants[, .(scale_awards, index_gender, index_discipline)]),
	N = NWOGrants[, .N],
...
```{r}
dag <- dagify(
	scale_awards ~ index_gender + index_discipline,
	index_discipline ~ index_gender,
	exposure = 'index_gender',
	outcome = 'scale_awards'
)
